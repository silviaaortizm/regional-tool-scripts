...
if ngroups_water == 0{
	gen double subsidy_water_direct = 0
	gen double exempted_cons_water = 0
	gen double spending_water = 0
}
else {
	local groupid = 0
	...
	forval i=1/5{
		...
		gen double subsidy`i'=.
		gen double spending`i'=.
	}
	...
	egen double subsidy_water_direct=rowtotal(subsidy*)
	egen double spending_water=rowtotal(spending*)
...
gen VATable_spend_water = spending_water - exempted_cons_water
...
if "${P4_3_cost_prof}${P4_3_tprof}" == "" {
	gen double water_ind_shock=0
	gen double water_tot_shock=0
}
else {
	
	*Shock
	gen double shock=(${P4_3_cost_prof}-${P4_3_tprof})*water_sec/${P4_3_cost_prof}
