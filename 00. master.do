...
if "`c(username)'"=="User" {
	global path     	"D:\OneDrive\Senegal\RegionalTool"
	global country		"GMB"
}
if "`c(username)'"=="wb419055" {
	global path     	"C:/Users/OneDrive/Senegal/RegionalTool"	
	global country		"MRT"
}
...
if "`c(username)'"=="Silvia" {
	...
	* Data
	...
	global param_out   	"${path}/01-Data/4_sim_output"
...
if (1) qui: include "${thedo}/02. Income Tax.do" 
else {
...
if (1) qui: include "${thedo}/03. Social Security Contributions.do" 
else {
	use hhid using "${presim}/01_menages.dta", clear
...
if (1) qui: include "${thedo}/04. Direct Transfers.do" 
else {
	use hhid using "${presim}/01_menages.dta", clear
...
if (1) qui: include "${thedo}/04. Indirect Taxes - Custom Duties.do" 
else {
	use hhid using "${presim}/01_menages.dta", clear
...
if (1) {
	qui: include "${thedo}/05a. Subsidies_Electricity.do"
	qui: include "${thedo}/05b. Subsidies_Fuels.do"
	qui: include "${thedo}/05c. Subsidies_Water.do"
	qui: include "${thedo}/05d. Subsidies_Agric.do"
	qui: include "${thedo}/05e. Subsidies_Merge.do"
}
else {
	use hhid using "${presim}/01_menages.dta", clear
...
if (1) qui: include "${thedo}/06. Indirect Taxes - Excises.do"
else {
	use hhid using "${presim}/01_menages.dta", clear
...
if (1) qui: include "${thedo}/07. Indirect Taxes - VAT.do" 
else {
	use hhid using "${presim}/01_menages.dta", clear
...
if (1) qui: include "${thedo}/08. In-Kind Transfers.do" 
else {
	use hhid using "${presim}/01_menages.dta", clear
...
qui: include "${thedo}/09. Income Aggregates.do" 
qui: include "${thedo}/09a. GenderSpatial Variables.do" 

*-------------------------------------
// 07. Process outputs
*-------------------------------------

qui: include "${thedo}/10. Outputs - Tool.do" 
set rmsg off
